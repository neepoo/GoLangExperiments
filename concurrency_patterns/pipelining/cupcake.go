...
func Bake(mixture string) string {
	...
	return fmt.Sprintf("cupcake in %s", mixture)
}

func AddToppings(backedCupCake string) string {
	fmt.Println("Adding topping to", backedCupCake)
	time.Sleep(everyThingElseTime * time.Second)
	return fmt.Sprintf("topping on %s", backedCupCake)
}
